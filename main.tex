\input{res/preamble.tex}

\begin{document}
\input{res/primera_pagina.tex}

\addtocontents{toc}{\setcounter{tocdepth}{1}}
\tableofcontents

\section{Template}
\cppfile{res/template.cpp}

\section{Binary \& Ternary Search}

\subsection{Binary search}
\cppfile{binary_ternary/binary_search.cpp}

\subsection{Binary lifting}
\cppfile{binary_ternary/binary_lifting.cpp}

% To-do: Revisar por qué no quiere funcionar con archivos con acentos.
\subsection{Binary search STL}
\cppfile{binary_ternary/bs_stl.cpp}

\subsection{Ternary search}
\cppfile{binary_ternary/ternary_search.cpp}

\section{Sorting}

\subsection{Merge sort}
\cppfile{sorting/merge_sort.cpp}

% To-do: Agregar quick sort.

\subsection{Sort STL}
\cppfile{sorting/sort_stl.cpp}

\section{Teoría de Números}

\subsection{Criterios de divisivilidad}
\input{number_theory/criterios_divisibilidad.tex}

\subsection{Operaciones modulares}
\cppfile{number_theory/operacionesModulares.cpp}

\subsection{GCD \& LCM}
\cppfile{number_theory/gcd_lcm.cpp}

\subsection{Cribas}

\subsubsection{Base}
\cppfile{number_theory/cribas/base.cpp}

\subsubsection{Vector de primos}
\cppfile{number_theory/cribas/return_primes.cpp}

\subsubsection{Descomposición factorial}
\cppfile{number_theory/cribas/descomposicion_primos.cpp}

\subsubsection{Criba de mayor divisor}
\cppfile{number_theory/cribas/mayor_divisor.cpp}

\subsubsection{Criba de número de divisores}
\cppfile{number_theory/cribas/numero_divisores.cpp}

\subsubsection{Criba de suma de divisores}
\cppfile{number_theory/cribas/suma_divisores.cpp}

\subsubsection{Divisores de un número}
\cppfile{number_theory/divisors.cpp}

\section{Gráfos}

\subsection{Breadth First Search (BFS)}
\cppfile{graphs/BFS.cpp}

\subsection{Depth First Search (DFS)}
\cppfile{graphs/DFS.cpp}

\subsection{Dijkstra}
\cppfile{graphs/Dijkstra.cpp}

\subsection{BFS-01}
\cppfile{graphs/bfs-01.cpp}

\subsection{Bellman Ford}
\cppfile{graphs/BellmanFord.cpp}

\section{Strings}

\subsection{Knuth-Morris-Pratt (KMP)}
\cppfile{strings/kmp.cpp}

\subsection{Subcadena más grande sin repetidos}
\cppfile{strings/longestSubstring.cpp}

% To-do: Pensar en un mejor nombre para esta clasificación
\section{Estructuras de Datos}

\subsection{Trie}
\cppfile{data_structures/trie.cpp}

\subsection{Fenwick Tree}
\cppfile{data_structures/fenwick_tree.cpp}

\subsection{Disjoint Set Union (DSU)}
\cppfile{data_structures/DSU.cpp}

\section{Misceláneo}

\subsection{Template INF}
\cppfile{misc/inf_template.cpp}

\subsection{AND}
\includegraphics[width=7cm]{and}

\subsection{OR}
\includegraphics[width=7cm]{or}

\subsection{XOR}
\includegraphics[width=7cm]{xor}

\subsection{Bitmask}
\cppfile{misc/bitmask.cpp}

\subsection{Ordered Set}
\cppfile{misc/ordered_set.cpp}

\subsection{Depuarador}
\input{misc/gdb_lldb.tex}

\subsection{Fechas}
\cppfile{misc/dates.cpp}

\section{Geometría}

\subsection{Trigonometría}
\cppfile{math/trigonometry.cpp}

\subsection{Implementaciones rápidas}
\cppfile{math/impl.cpp}


\end{document}
